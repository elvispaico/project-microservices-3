...
public class ProductServiceImpl implements ProductService {
    ...
    @Override
    public Mono<Product> save(Product request) {
        return customerApi.findCustomerById(request.getIdCustomer())
                .switchIfEmpty(Mono.error(new ResourceNotFoundException("Customer not found")))
                .flatMap(customer -> {
                    return hasProductExpired(request.getIdCustomer())
                            .flatMap(hasDebt -> {
                                if (!hasDebt) {
                                    ...
                                    return Mono.error(new AttributeException("Customer have expired debt"));
                                }
    ...
    @Override
    public Flux<Product> findAllProductByIdCustomer(String idCustomer) {
        return productRepository.findAllByIdCustomer(idCustomer);
    }

    @Override
    public Mono<ReportTransactionsResponse> findProductWhitTransactionCommission(String idProduct) {

        return productRepository.findById(idProduct)
                .switchIfEmpty(Mono.error(new ResourceNotFoundException("Product not fount")))
                .flatMap(product -> transactionRepository.findByIdProduct(product.getId())
                        .collectList()
                        .flatMap(lisTransations -> {
                            ...
                            var totalCommission = newList.stream()
                                    ...
                                    .sum();
                            var report = ReportTransactionsResponse.builder()
    ...
    @Override
    public Mono<Product> saveAccountPersonalVip(Product product) {

        return customerApi.findCustomerById(product.getIdCustomer())
                .switchIfEmpty(Mono.error(new ResourceNotFoundException("Product not fount")))
                .flatMap(customer -> {
                    if (!BussinessLogic.isCustomerPersonalVip(customer.getCodTypeCustomer(), customer.getProfile().getCodPerfil())) {
                        return Mono.error(new AttributeException("Customer is not a Vip"));
                    } else {
                        return productRepository.existsProductCard(customer.getId())
                                .flatMap(existProduct -> {
                                    if (!existProduct) {
                                        return Mono.error(new AttributeException("Card no exist"));
                                    } else {
    ...
    private Mono<Product> saveProductCustomerBussines(Product request, CustomerResponse customer) {
        if ((isVerifyCustomerBussinessAccountSuccess(request) && isVerifyCommissionSuccess(request))
                        ...
                        .flatMap(existProduct -> {
                            if (!existProduct) {
                                ...
                                return productRepository.existsProductAccountCurrent(customer.getId())
                                        .flatMap(existProduct2 -> !existProduct2 ? Mono.error(new AttributeException("Account current no exist"))
            ...
            return Mono.error(new AttributeException("Operacion no disponible"));
        }
    }

    private Mono<Product> saveProductCustomerPersonal(CustomerResponse customer, Product request) {
        return productRepository.findByCustomer(request.getIdCustomer())
                    ...
                    if (!isValue) {
                        if (isVerifyCommissionSuccess(request)) {
                            if (BussinessLogic.isCustomerPersonalVip(customer.getCodTypeCustomer(), customer.getProfile().getCodPerfil())
                                    && BussinessLogic.isServiceAccountSaving(request.getCodTypeService())) {
                                return productRepository.existsProductCard(customer.getId())
                                        .flatMap(existProduct -> !existProduct ? Mono.error(new AttributeException("Card no exist"))
                                                : productRepository.save(request));
                            } else {
                                return productRepository.save(request);
                            }
                        } else {
                            return Mono.error(new AttributeException("Invalid commission"));
                        }
                    } else {
                        return Mono.error(new AttributeException("Client already has registered accounts"));
                    }
                });
    }
    ...
    public Mono<Boolean> hasProductExpired(String idCustomer) {
        return productRepository.findAllByIdCustomer(idCustomer)
                .filter(BussinessLogic::flProductHaveExpiredDebts)
                .collectList()
                .flatMap(products -> products.isEmpty() ? Mono.just(false)
                        : Mono.just(true));
    }
}
